...
public class JWTAuthFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if(authenticationHeader != null && authenticationHeader.startsWith(JWTConst.BEARER_NAME)) {

				final String bearerTkn= authenticationHeader.replaceAll((JWTConst.BEARER_NAME+ " "), "");
				System.out.println("Following token is received from the protected url= "+ bearerTkn);

				try {
					// Parsing the jwt token.
					Jws<Claims> claims = Jwts.parser().requireIssuer(JWTConst.ISSUER_NAME).setSigningKey(JWTConst.SECRET_KEY).parseClaimsJws(bearerTkn);
